...
void* GetElementVector(const Vector *v, size_t i) {
    if (v==NULL) {
        fprintf(stderr, "Нельзя узнать адрес элемента у NULL\n");
        return NULL;
    }
    if (v->size < i) {
        fprintf(stderr, "Нет такого элемента в векторе\n");
        return NULL;
    }
...
void SetElementVector(Vector *v, size_t i, void *new) {
    if (v==NULL || new == NULL) {
        fprintf(stderr, "Нельзя узнать адрес элемента у NULL или новый элемент NULL\n");
        return;
    }
    if (v->size < i) {
        fprintf(stderr, "Нет такого элемента в векторе\n");
    ...
    return v->type_info->assign(new, GetElementVector(v, i));
}
...
Vector* CreateVector (size_t size, const TypeInfo *type_info) { //скорее всего попросят передать выделение памяти пользователю
    ...
    for (size_t i = 0; i < size; i++ ) {
        type_info->assign(zero, GetElementVector(v, i));
    }
    return v;
}
...
void FillVector (Vector *v, void **values, size_t values_size) {
    ...
    for (size_t i = 0; i < values_size; i++) {
        if (values[i] == NULL) {
            ...
            for (size_t j = 0; j < i; j++) (ti->assign(ti->zero(), GetElementVector(v, j)));
            return;
        }
        ti->assign(values[i], GetElementVector(v, i));
...
void AddVector (const Vector *v1, const Vector *v2, Vector *v_res) {
    ...
    for (size_t i = 0; i < v1->size; i++) {
        ti->add(GetElementVector(v1, i), GetElementVector(v2, i), GetElementVector(v_res, i));
    }
}
...
void DotVector (const Vector *v1, const Vector *v2, void *result) {
    ...
    for (size_t i = 0; i < v1->size; i++ ) {
        typeinfo->multiply(GetElementVector(v1, i), GetElementVector(v2, i), mul_res);
...
void NormVector (const Vector *v, double *result) {
    ...
    for (size_t i = 0; i < v->size; i++) {
        v->type_info->square(GetElementVector(v, i), sq);
...
void PrintVector (const Vector *v) {
    ...
    printf("[");
    for (size_t i = 0; i < v->size; i++) {
        v->type_info->print(GetElementVector(v, i));
